...
class NgosList extends Component {
  ...
  render() {
    const { isSuperUserSummary,supportServiceWorker } = this.context;
    ...
    let ngolist = ngos.map((ngo) => {
      return (
        <Card key={ngo.uuid} style={{ marginBottom: ".5em", padding: "1em" }}>
      ...
      <div className="ngolist-Section">
        ...
        {isAlreadyAuthenticated ? (
          <div className="ngoSection">
            <Title title="NgoList" />
            <div className="ngoInnerSection">{ngolist}</div>
          </div>
        ) : (
